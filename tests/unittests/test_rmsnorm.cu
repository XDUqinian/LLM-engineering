...
bool CheckResult(float* CPUoutput, T* GPUoutput, int output_size) {
    ...
    for (int i = 0; i < output_size; i++) {
        ...
        if (fabs(CPUoutput[i] - fp32GPUoutput) > 1e-5) {
            printf("Error! CPUoutput[%d] = %f, GPUoutput[%d] = %f\n", i, CPUoutput[i], i, fp32GPUoutput);
            return false;
        }
    }
    return true;
...
int main(int argc, char *argv[]) {
    ...
    float eps = 1e-6;
    if (argv[1]) {
        half* h_decoder_out = (half*)malloc(total_size * sizeof(half));
        half* decoder_out = (half*)malloc(total_size * sizeof(half));
        half* d_decoder_out;
        cudaMalloc((void**)&d_decoder_out, total_size * sizeof(half));
        for (int i = 0; i < total_size; i++) h_decoder_out[i] = (half)1.0f;
        half* d_decoder_rsd;
        cudaMalloc((void**)&d_decoder_rsd, total_size * sizeof(half));
        half* h_scale = (half*)malloc(hidden_units * sizeof(half));
        half* d_scale;
        cudaMalloc((void**)&d_scale, hidden_units * sizeof(half));
        for (int i = 0; i < hidden_units; i++) h_scale[i] = (half)1;

        CHECK(cudaMemcpy(d_decoder_out, h_decoder_out, total_size * sizeof(half), cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(d_scale, h_scale, hidden_units * sizeof(half), cudaMemcpyHostToDevice));

        DataType type_half = getTensorType<half>();
        DataType type_int = getTensorType<float>();
        TensorWrapper<half>* decoder_out_tensor = new TensorWrapper<half>(Device::GPU, type_half, {num_tokens, hidden_units}, d_decoder_out);
        TensorWrapper<half>* decoder_residual_tensor = new TensorWrapper<half>(Device::GPU, type_half, {num_tokens, hidden_units}, d_decoder_rsd);
        LayerNormWeight<half> scale;
        scale.gamma = d_scale;
        std::cout << "before launch kernel" << std::endl;
        launchRMSNorm(decoder_out_tensor, decoder_residual_tensor, scale, eps);
        ...
        std::cout << "rmsnorm passed "<< is_right << std::endl;
        free(h_decoder_out);
        free(h_scale);
        free(cpu_scale);
        free(CPUout);
