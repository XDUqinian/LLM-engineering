...
void cpuEmbedding(const int* input_ids, half* output, half* embed_table, const int max_context_token_num, const int hidden_size, const int vocab_size) {
    for (int i = 0; i < max_context_token_num; i++) {
        for (int j = 0; j < hidden_size; j++) {
            output[i * hidden_size + j] = embed_table[input_ids[i] * hidden_size + j];
        }
...
bool checkResults(half* h_output, half* d_output, const int output_size) {
    half* d_output_cpu = (half*) malloc(output_size * sizeof(half));
    CHECK(cudaMemcpy(d_output_cpu, d_output, output_size * sizeof(half), cudaMemcpyDeviceToHost));
    for (int i =0; i < output_size; i++) {
        if (fabs((float)d_output_cpu[i] - (float)h_output[i]) > 1e5) {
            std::cout << "Dev : ";
            for (int j = max(0, i - 10); j < min(output_size, i + 10); j++) {
                std::cout << (float)d_output_cpu[i] <<" ";
            }
            std::cout << std::endl;
            std::cout << "Cpu : ";
            for (int j = max(0, i - 10); j < min(output_size, i + 10); j++) {
...
int main(int argc, char *argv[]) {
    ...
    if (argv[1]) {
        ...
        CHECK(cudaMalloc((void**)&d_input, input_size * sizeof(int)));
        CHECK(cudaMalloc((void**)&d_table, table_size * sizeof(float)));
        ...
        if (checkResults(h_output, output->data, output_size)) std::cout << "True\n";
        else std::cout << "False\n";
