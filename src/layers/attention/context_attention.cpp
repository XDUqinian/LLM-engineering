...
void LLaMAContextAttentionLayer<T>::allocForForward(LLaMAAttentionDynParams& params) {
    ...
    q_buf_w_pad = new TensorWrapper<T>(Device::GPU, type, {batch_size, head_num, max_q_len, head_size});// pad to max_q_len that is max len of cur batch
...
{
    // allocate intermediat buf of the layer forward
    ...
    //note: qkv bias is not existed in llama
