...
LLaMASelfAttentionLayer<T>::LLaMASelfAttentionLayer(
    ...
    // TODO: check kv_head_num is divided by head_num
...
void LLaMASelfAttentionLayer<T>::allocForForward(LLaMAAttentionDynParams& params) {
    ...
    // note: 当前step的q k v的shape里面step或seqlen都是1，之前step的kv在做gemv的时候直接从kv cache拿
...
// notes: params order of launcher function in LaMAContextAttentionLayer<T>::forward: (input[Tensor], input[Tensor],...,weight[Weight], output[*])
template <typename T>
...
{
    // note: allocate intermediate buf of the layer forward
    ...
    // kv cahce shape = [bs, kv head num, max seq len, head size]
    ...
    DeviceSyncAndCheckCudaError();
    // 3. fused masked mha
    launchDecoderMaskedMHA<T>(qkv_buf,
                              weights.qkv,
                              layer_id->as<int>(),
                              key_cache->as<T>(),
                              value_cache->as<T>(),
    ...
    launchLinearGemm(mha_output,
                     weights.output,
                     attention_output->as<T>(),
                     cublas_wrapper,
                     false,
                     true);
    DeviceSyncAndCheckCudaError();
