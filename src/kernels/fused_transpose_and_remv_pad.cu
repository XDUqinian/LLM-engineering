                                                 ...
                                                 const int *padding_offset)
{
    ...
    int seqlen_id = (token_id + padding_offset[token_id]) % seq_len;

    int src_offset = batch_id * head_num * seq_len * head_size +
                     seqlen_id * head_size;
    ...
    for (int i = threadIdx.x; i < head_num * head_size; i += blockDim.x)
    {
        ...
        int tid = i % head_size;
        dst[dst_offset + i] = src[src_offset + head_id * seq_len * head_size + tid];
    }
}
template <typename T>
