...
// Note: a b 两个 topK reduce 输出一个 topK
...
__global__ void topK_kernel_round1(const T* probs, const int vocab_size, int* topK_ids, T* topK_vals)
{
    typedef cub::BlockReduce<topK<T, K>, blockSize> blockreduce;
    __shared__ typename blockreduce::TempStorage tem_storage;

    int tid = threadIdx.x;
    int bid = blockIdx.x;
    int gid = blockIdx.x * blockDim.x + threadIdx.x;
    int beam_id = bid / BlockPerBeam;
    ...
    for(int data_id = tid + block_lane * blockSize; data_id < vocab_size; data_id += BlockPerBeam * blockSize){
        int data_offset = data_id + beam_id * vocab_size;
        T data = probs[data_offset];
        thread_topK.insertHeap(data, data_id);
    }
    topK<T, K> block_topK = blockreduce(tem_storage).Reduce(thread_topK, reduce_functor<T, K>);
    if (tid == 0){
        for(int k_offset = 0; k_offset < K; k_offset++) {
            topK_vals[beam_id * BlockPerBeam * K + block_lane * K + k_offset] = block_topK.val[k_offset];
            topK_ids[beam_id * BlockPerBeam * K + block_lane * K + k_offset] = block_topK.id[k_offset];
        }
    }
}
// shape infer: [bs, beamwidth, BlockPerBeam, K] => [bs, beamwidth, K]
// ids是beam width * vocab size中的全局word id
// gridSize = bs * beamwidth
// blockSize = 256
template <typename T, int K, int blockSize, int BlockPerBeam>
__global__ void topK_kernel_round2(const int* topK_ids, const T* topK_vals, int* final_topK_ids, T* final_topK_vals)
{
    typedef cub::BlockReduce<topK<T, K>, blockSize> blockreduce;
    ...
    for(int i = tid; i < BlockPerBeam * K; i += blockDim.x) {
        int data_offset = beam_id * BlockPerBeam * K + i;
    ...
    if(tid == 0){
        for(int k_offset = 0; k_offset < K; k_offset++) {
            ...
            final_topK_vals[beam_id * K + k_offset] = block_topK.val[k_offset];
            final_topK_ids[beam_id * K + k_offset] = block_topK.id[k_offset];
        }
    }    
}
...
constexpr int beamwidth = 1; // 因为做 sampling 而不是 beamSearch
constexpr int K = 5;
// buffer size
...
dim3 block_roud1(256);
dim3 grid_round2(BlockNums2);
dim3 block_roud2(256);
topK_kernel_round1<T, K, 256, BlockPerBeam><<<grid_round1, block_roud1>>>(probs->data, vocab_size, topK_ids, topK_vals);
topK_kernel_round2<T, K, 256, BlockPerBeam><<<grid_round2, block_roud2>>>(topK_ids, topK_vals, final_topK_ids, final_topK_vals);
}
