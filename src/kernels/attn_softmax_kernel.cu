                                          ...
                                          float scale)
{
    ...
    for (int row_start = blockIdx.x; row_start < q_len; row_start += gridDim.x)
    {
        ...
        // 面对这种一个block一个thread需要处理多行多列的时候，数据尽量用数组存储，计算出每个block和thread要处理几行几列
        ...
        for (int col_start = 0; col_start < NUMS_PER_THREAD_PER_ROW; col_start++)
        {
            ...
            data[col_start] = scale * qk_data + (1 - mask_data) * (-10000.f);
        ...
        for (int col_start = 0; col_start < NUMS_PER_THREAD_PER_ROW; col_start++)
        {
            qk_offset = batch_id * head_nums * q_len * k_len +
                        ...
                        row_start * k_len +
                        col_start * blockDim.x + threadIdx.x;
            attn_score[qk_offset] = inv_sum * data[col_start];
        }
    }
}
                                         ...
                                         float scale)
{
    ...
    Vec_t* mask_vec = reinterpret_cast<Vec_t*>(mask);
    Vec_t ONE = scalar_cast_vec<Vec_t>(__float2half(1.0f));
    Vec_t NEG_INF = scalar_cast_vec<Vec_t>(__float2half(-10000.f));
    Vec_t scale_vec = scalar_cast_vec<Vec_t>(__float2half(scale));

    if(threadIdx.x * vec_size >= k_len) return;
    ...
    for (int row_start = blockIdx.x; row_start < q_len; row_start +=gridDim.x)
    {
        ...
        for (int col_start = 0; col_start < NUMS_PER_THREAD_PER_ROW; col_start++)
        {
            qk_offset = batch_id * head_nums * q_len * k_len / 2 +
                        head_id * q_len * k_len / 2 +
                        row_start * k_len / 2 +
                        col_start * blockDim.x + threadIdx.x;
            qk_data = qk_vec[qk_offset];
            mask_offset = batch_id * q_len * k_len / 2 +
                          row_start * k_len / 2 +
                          col_start * blockDim.x + threadIdx.x;
            mask_data = mask_vec[mask_offset];
            data[col_start] = __hadd2(__hmul2(scale_vec, qk_data), __hmul2(__hsub2(mask_data, ONE), NEG_INF));
