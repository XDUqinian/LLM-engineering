                                            ...
                                            int hidden_units)
{
    ...
    Vec_t *rsd, *bia, *s;
    Vec_t tmp;
    Vec_t* de_out = reinterpret_cast<Vec_t*>(decoder_out + token_id * hidden_units);
    if (residual != nullptr) {
        rsd = reinterpret_cast<Vec_t*>(residual + token_id * hidden_units);
    }
    if(bias != nullptr) {
    ...
    __syncthreads();
    for (int i = tid; i < hidden_units / vec_size; i += blockDim.x){
        de_out[i].x  = de_out[i].x * inv_fenmu * s[i].x;
        de_out[i].y  = de_out[i].y * inv_fenmu * s[i].y;
        de_out[i].z  = de_out[i].z * inv_fenmu * s[i].z;
        de_out[i].w  = de_out[i].w * inv_fenmu * s[i].w;
    }
}
