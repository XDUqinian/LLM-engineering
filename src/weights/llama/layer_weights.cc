    ...
    attn_bias(attn_bias)
{
    ...
    // note: we concat gate linear weight and up linear weight to one weight tensor for performance improvement
...
// note: weight from HF is always half type, and if we want run fp32 inference, we should convert half weight to fp32 weight in tools/weights_convert.py
// note: shape and data of ffn weight downloaded from HF are transposed, so we should carefully declare shape here
template <typename T>
void LlamaLayerWeight<T>::loadWeights(std::string weight_path, WeightType weight_type) // weight type 参数比较多余
{
...
// note: load dummy model/weight API, is used to the time when you want test inference performance only
template <typename T>
